// src/app/compartido/clinker_fortaleza/[token]/page-client.js
"use client";
...
const COLORES_GRAFICOS = [COLOR_AZUL_PRINCIPAL, COLOR_AZUL_MARINO, COLOR_VERDE_GRIS, "#14B8A6", "#3B82F6", "#6B7280"]
...
function useClinkerFortalezaData(token, transporteFiltro = null, diaFiltro = null, destinoFiltro = null) {
  ...
  const cargar = async () => {
    try {
      ...
      if (productoError || !productoData) throw new Error('Producto CLINKER FORTALEZA no encontrado')
      ...
      const registrosRaw = await CARGAR_TODOS_LOS_REGISTROS('clinker_fortaleza_viajes', 'barco_id', barcoData.id)
...
export default function ClientPage({ token }) {
  ...
  const { barco, producto, registros, destinos, loading, error, lastUpdate, refetch } = useClinkerFortalezaData(
    ...
    const cargarTodosRegistros = async () => {
      try {
        ...
        if (barcoData) {
          ...
          const destinosMap = new Map()
          destinosData?.forEach(d => destinosMap.set(d.id, d))
          const registrosGlobales = await CARGAR_TODOS_LOS_REGISTROS('clinker_fortaleza_viajes', 'barco_id', barcoData.id)
  ...
  const S = {
    header: (align = 'center') => ({
      ...
      fill: { fgColor: { rgb: '14B8A6' }, patternType: 'solid' },
  ...
  // GUARDAR ARCHIVO
  // ─────────────────────────────────────────────────────────────
  const nombreArchivo = `Clinker_Fortaleza_${barco?.nombre || 'descarga'}_${dayjs().tz(ZONA_HORARIA_SV).format('YYYY-MM-DD_HHmm')}.xlsx`
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: COLOR_GRIS_FONDO }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: COLOR_AZUL_PRINCIPAL, fontWeight: '500', fontSize: '14px', letterSpacing: '1px' }}>CARGANDO DATOS DE CLINKER FORTALEZA</p>
        </div>
      </div>
    )
  }
    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alm-row-bajo { background: linear-gradient(90deg, rgba(20, 184, 166, 0.06), transparent); border-left: 3px solid var(--teal); }
      ...
      <div className="alm-clinker-fortaleza-root">
        <header className="alm-topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div>
              ...
              <div className="alm-ship-code">#{barco.codigo_barco} · Clinker Fortaleza CLF-001</div>
            </div>
          </div>
        ...
        <div className="alm-body">
          ...
          <div style={{ textAlign: 'center', padding: '24px 20px', borderTop: `1px solid ${COLOR_BORDE}`, marginTop: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '24px', flexWrap: 'wrap', marginBottom: '14px', fontSize: '10px', color: COLOR_TEXTO_SECUNDARIO }}>
              ...
              <span><GiMinerals size={10} /> Clinker Fortaleza CLF-001</span>
            ...
            <div style={{ marginTop: '14px', fontSize: '9px', color: COLOR_AZUL_PRINCIPAL, fontWeight: '500' }}>
              ALMACENADORA DEL PACÍFICO · Sistema de Gestión de Descarga de Clinker Fortaleza
            </div>
          </div>
        </div>
      </div>
