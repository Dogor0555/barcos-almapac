...
import { Ship, LogOut, Anchor, AlertCircle, RefreshCw, Clock, Play } from 'lucide-react'
...
export default function ChequeroNicaPage() {
  ...
  const seleccionarBarco = (token) => {
    router.push(`/barco/${token}/clinker-nica`)
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-4 md:p-8">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-orange-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5 animate-pulse"></div>
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-red-500 rounded-full mix-blend-multiply filter blur-3xl opacity-5 animate-pulse"></div>
      </div>
        ...
        <div className="bg-gradient-to-r from-orange-600 to-red-600 rounded-2xl p-6 text-white shadow-2xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">Chequero Clinker Nicaragua</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {barcos.map((barco) => (
              <button
                ...
                className="bg-[#1e293b] border border-white/10 hover:border-orange-500/40 hover:bg-orange-500/5 rounded-2xl p-6 text-left transition-all group shadow-lg hover:shadow-orange-500/10"
              >
                {/* Header de la card */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="bg-orange-500/10 group-hover:bg-orange-500/20 p-3 rounded-xl transition-all">
                      <Ship className="w-6 h-6 text-orange-400" />
                    </div>
                    <div>
                      <h2 className="text-lg font-black text-white group-hover:text-orange-300 transition-colors">
                        {barco.nombre}
                      </h2>
                      {barco.codigo_barco && (
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    <Anchor className="w-3.5 h-3.5 text-orange-400 flex-shrink-0" />
                    <span className="text-slate-400">Clinker - Nicaragua</span>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                  <span className="text-xs text-slate-600">
                    Toca para gestionar operación
                  </span>
                  <span className="text-xs font-bold text-orange-400 group-hover:translate-x-1 transition-transform inline-block">
                    Ver →
                  </span>
                </div>
              </button>
