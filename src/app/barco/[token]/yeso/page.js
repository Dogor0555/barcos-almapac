// src/app/barco/[token]/yeso/page.js
'use client'
...
export default function YesoPage() {
  ...
  const cargarDatos = useCallback(async (mostrarToast = false) => {
    ...
    try {
      ...
      const [barcoRes, productoRes] = await Promise.all([
        ...
        supabase.from('productos').select('*').eq('codigo', 'YE-001').single()
      ])
      ...
      if (productoError || !productoData) {
        if (mostrarToast) toast.error('Producto YESO no encontrado')
        return
      }
      ...
      // Meta del producto
      const metaProducto = barcoData.metas_json?.limites?.['YE-001'] || 0
      setMeta(metaProducto)

      // 🔥 CONSULTA SIN LÍMITE - TRAE TODOS LOS REGISTROS
      const viajesData = await CARGAR_TODOS_LOS_REGISTROS('yeso_viajes', 'barco_id', barcoData.id)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime-yeso-${barco.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const handleEliminar = async (id, correlativo) => {
    ...
    try {
      const { error } = await supabase.from('yeso_viajes').delete().eq('id', id)
      if (error) throw error
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0f172a]">
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 text-center max-w-md">
          ...
          <p className="text-slate-400">
            {!barco ? 'Barco no encontrado' : 'Producto YESO no configurado'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-orange-200 text-sm mt-1">
                Registro de descarga de yeso - {new Date().toLocaleDateString()}
              </p>
            </div>
        ...
        <div className="bg-slate-800/30 border border-white/5 rounded-xl p-4">
          <div className="flex flex-col gap-2 text-sm text-slate-400">
            <p className="font-bold text-orange-400">PROCESO DE REGISTRO - YESO:</p>
