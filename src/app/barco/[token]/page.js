...
export default function BarcoPesadorPage() {
  ...
  const [buscarPlacaTiempos, setBuscarPlacaTiempos] = useState('')
...
// Función para calcular el acumulado destino (suma acumulativa de peso_destino_tm)
const calcularAcumuladoDestino = (viajesList, viajeId, orden) => {
  if (!viajesList.length) return '0.000'
  
  let viajesOrdenados = [...viajesList]
  if (orden === 'asc') {
    viajesOrdenados.sort((a, b) => a.viaje_numero - b.viaje_numero)
  } else {
    viajesOrdenados.sort((a, b) => b.viaje_numero - a.viaje_numero)
  }
  
  const index = viajesOrdenados.findIndex(v => v.id === viajeId)
  if (index === -1) return '0.000'
  
  let acumulado = 0
  for (let i = 0; i <= index; i++) {
    const peso = Number(viajesOrdenados[i].peso_destino_tm) || 0
    acumulado += peso
  }
...
const calcularTiemposEntreViajes = () => {
  ...
  // Filtrar resultados por placa
const resultadosFiltradosTiempos = useMemo(() => {
  if (!tiemposResultados.length) return []
  if (!buscarPlacaTiempos.trim()) return tiemposResultados
  const termino = buscarPlacaTiempos.trim().toUpperCase()
  return tiemposResultados.filter(item => item.placa.toUpperCase().includes(termino))
}, [tiemposResultados, buscarPlacaTiempos])
        ...
        <table className="w-full">
          <thead className="bg-slate-800 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-bold text-slate-400 uppercase whitespace-nowrap">Acumulado Destino</th>
          ...
          <tbody className="divide-y divide-white/5">
            {viajesFiltrados.length === 0 ? (
              ...
              if (ordenViajes === 'asc') {
                ...
                acumuladoCorridoUPDP = viajesAsc
                  .slice(0, idxAsc + 1)
                  .reduce((sum, v) => sum + (Number(v.peso_neto_updp_tm) || 0), 0)
              }

              // Calcular acumulado Destino
              let acumuladoCorridoDestino = 0
              if (ordenViajes === 'asc') {
                acumuladoCorridoDestino = array
                  .slice(0, index + 1)
                  .reduce((sum, v) => sum + (Number(v.peso_destino_tm) || 0), 0)
              } else {
                const viajesAsc = [...viajesFiltrados].sort((a, b) => a.viaje_numero - b.viaje_numero)
                const idxAsc = viajesAsc.findIndex(v => v.id === viaje.id)
                acumuladoCorridoDestino = viajesAsc
                  .slice(0, idxAsc + 1)
                  .reduce((sum, v) => sum + (Number(v.peso_destino_tm) || 0), 0)
              }
              ...
              return (
                <tr key={viaje.id} className={`hover:bg-white/5 transition-colors ${haySaltoDespues ? 'border-b-2 border-amber-500/50' : ''}`}>
                  ...
                  <td className="px-4 py-3 font-bold text-yellow-400 whitespace-nowrap">{acumuladoCorridoUPDP.toFixed(3)}</td>
                  <td className="px-4 py-3 font-bold text-teal-400 whitespace-nowrap">{acumuladoCorridoDestino.toFixed(3)}</td>
          ...
          <tfoot className="bg-slate-900 border-t border-white-10 sticky bottom-0">
            <tr>
              ...
              <td className="px-4 py-3 font-bold text-teal-400 whitespace-nowrap">
                {viajesFiltrados.reduce((sum, v) => sum + (Number(v.peso_destino_tm) || 0), 0).toFixed(3)}
              </td>
              <td className="px-4 py-3 whitespace-nowrap">—</td>
              <td className="px-4 py-3 whitespace-nowrap">—</td>
          ...
          <>
            ...
            <div className="mb-4">
              <div className="relative">
                <input
                  type="text"
                  value={buscarPlacaTiempos}
                  onChange={(e) => setBuscarPlacaTiempos(e.target.value.toUpperCase())}
              ...
              <p className="text-xs text-slate-500 mt-1">
                {resultadosFiltradosTiempos.length} de {tiemposResultados.length} unidades encontradas
              </p>
            </div>
            ...
            {resultadosFiltradosTiempos.length === 0 ? (
              <div className="text-center py-12 bg-slate-900/50 rounded-xl border border-white/10">
                <Search className="w-12 h-12 text-slate-700 mx-auto mb-3" />
                <p className="text-slate-400">No se encontraron placas con "{buscarPlacaTiempos}"</p>
                <button
                  onClick={() => setBuscarPlacaTiempos('')}
              ...
              resultadosFiltradosTiempos.map((placaData, idx) => (
