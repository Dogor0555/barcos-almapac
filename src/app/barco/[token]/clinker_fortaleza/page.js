// src/app/barco/[token]/clinker_fortaleza/page.js
'use client'
...
export default function ClinkerFortalezaPage() {
  ...
  const [destinos, setDestinos] = useState([])  // 🔥 Destinos desde la tabla
  ...
  const [entrada, setEntrada] = useState({
    ...
    tipo_unidad: '',
    transporte: '',
    destino_id: '',      // 🔥 Cambiado de patio a destino_id
    bodega_barco: '',
  })
  ...
  const cargarDatos = useCallback(async (mostrarToast = false) => {
    ...
    try {
      ...
      // Meta del producto
      const metaProducto = barcoData.metas_json?.limites?.['CLF-001'] || 0
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime-clinker-fortaleza-${barco.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'clinker_fortaleza_viajes',
  ...
  const viajesCompletadosFiltrados = useMemo(() => {
    ...
    if (buscarEnTabla.trim()) {
      ...
      filtrados = filtrados.filter(v => 
        ...
        v.destino_info?.nombre?.toLowerCase().includes(termino)
      )
    }
  ...
  const destinoSeleccionado = destinos.find(d => d.id === entrada.destino_id)
  ...
  const handleRegistrarEntrada = async () => {
    ...
    if (!entrada.destino_id) return toast.error('El Destino es obligatorio')
    ...
    const nuevoViaje = {
      ...
      destino_id: entrada.destino_id,  // 🔥 Usar destino_id
  ...
  const handleDestinoSelect = (opt) => setEntrada(prev => ({ ...prev, destino_id: opt?.value || '' }))
  ...
  const guardarEdicion = async () => {
    ...
    const datosActualizar = {
      ...
      destino_id: viajeEnEdicion.destino_id || null,
  ...
  const handleEliminar = async (id, correlativo) => {
    ...
    try {
      const { error } = await supabase.from('clinker_fortaleza_viajes').delete().eq('id', id)
      if (error) throw error
  ...
  const selectStyles = {
    control: (base, state) => ({
      ...
      borderColor: state.isFocused ? '#14b8a6' : 'rgba(255,255,255,0.1)',
      borderRadius: '0.5rem',
    ...
    option: (base, state) => ({
      ...
      color: state.isSelected ? '#14b8a6' : '#cbd5e1',
  ...
  const opcionesDestinos = destinos.map(d => ({
    value: d.id,
    label: `${d.codigo} - ${d.nombre} (${d.tipo})`,
    tipo: d.tipo,
    codigo: d.codigo,
    nombre: d.nombre
  }))
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0f172a]">
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 text-center max-w-md">
          ...
          <p className="text-slate-400">
            {!barco ? 'Barco no encontrado' : 'Producto CLINKER FORTALEZA no configurado'}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-3xl font-black flex items-center gap-2">
                  ...
                  Clinker Fortaleza (CLF-001) - {barco.nombre}
                </h1>
                {barco.codigo_barco && (
              ...
              <p className="text-teal-200 text-sm mt-1">
                Registro de descarga de clinker fortaleza - {new Date().toLocaleDateString()}
              </p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <div className="bg-teal-500/30 px-4 py-2 rounded-xl font-bold flex items-center gap-2">
        ...
        {meta > 0 && (
          <div className={`rounded-2xl p-6 transition-all ${
            estaCompleto
              ...
              : estaCerca
              ? 'bg-gradient-to-r from-yellow-600/20 to-cyan-800/20 border border-yellow-500/30 animate-pulse'
              : 'bg-gradient-to-r from-teal-600/20 to-cyan-800/20 border border-teal-500/30'
          }`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                <div className={`p-3 rounded-xl ${
                  estaCompleto ? 'bg-green-500/20' : estaCerca ? 'bg-yellow-500/20' : 'bg-teal-500/20'
                }`}>
                  {estaCompleto ? <CheckCircle className="w-6 h-6 text-green-400" /> : <Target className="w-6 h-6 text-teal-400" />}
                </div>
                <div>
              ...
              <div className="text-right">
                ...
                <p className={`text-3xl font-black ${
                  estaCompleto ? 'text-green-400' : estaCerca ? 'text-yellow-400' : 'text-teal-400'
                }`}>
            ...
            <div className="space-y-2">
              ...
              <div className="h-3 bg-slate-700 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${
                  estaCompleto ? 'bg-green-500' : estaCerca ? 'bg-yellow-500' : 'bg-teal-500'
        ...
        {/* Acumulado */}
        <div className="bg-gradient-to-r from-teal-600/20 to-cyan-600/20 border border-teal-500/30 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-teal-400 uppercase font-bold">ACUMULADO ACTUAL</p>
              <p className="text-3xl font-black text-teal-400">{acumuladoActual.toFixed(3)} TM</p>
              <p className="text-[10px] text-teal-300/70 mt-0.5">Total descargado hasta el momento</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-yellow-400 font-bold focus:outline-none focus:border-teal-500" 
              />
            ...
            <div className="md:col-span-2">
              <label className="block text-xs text-slate-400 mb-1">Destino (Bodega/Silo/Bin/Módulo)</label>
              <Select 
                options={opcionesDestinos} 
                onChange={handleDestinoSelect} 
                value={destinoSeleccionado ? { value: destinoSeleccionado.id, label: `${destinoSeleccionado.codigo} - ${destinoSeleccionado.nombre} (${destinoSeleccionado.tipo})` } : null} 
                placeholder="Seleccionar destino..." 
                isClearable={false} 
                styles={selectStyles} 
              />
              <p className="text-[10px] text-slate-500 mt-0.5">Selecciona la bodega, silo, bin o módulo de destino</p>
            </div>
            
            <div>
              <label className="block text-xs text-slate-400 mb-1">Bodega Barco (Opcional)</label>
              <Select options={opcionesBodega} onChange={handleBodegaSelect} value={opcionesBodega.find(opt => opt.value === entrada.bodega_barco) || null} placeholder="Seleccionar bodega" isClearable styles={selectStyles} />
            </div>
            
            <div className="col-span-full">
              <button onClick={handleRegistrarEntrada} disabled={!entrada.placa || !entrada.tipo_unidad || !entrada.hora_entrada || !entrada.destino_id || !entrada.peso_bruto || !entrada.correlativo}
                className={`w-full font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition-all ${!entrada.placa || !entrada.tipo_unidad || !entrada.hora_entrada || !entrada.destino_id || !entrada.peso_bruto || !entrada.correlativo ? 'bg-slate-700 cursor-not-allowed text-slate-400' : 'bg-green-600 hover:bg-green-700 text-white'}`}>
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            {viaje.destino_info && (
                              <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">
                                {viaje.destino_info.codigo} - {viaje.destino_info.nombre}
                              </span>
                            )}
                          </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-white">
                      {viajeActivo.destino_info ? `${viajeActivo.destino_info.codigo} - ${viajeActivo.destino_info.nombre}` : '—'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-slate-900/50 border border-white/10 rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex gap-3">
                ...
                <div className="relative">
                  <input 
                    ...
                    placeholder="Buscar por placa, viaje, destino..." 
                    className="w-full md:w-80 bg-slate-800 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-white text-sm focus:outline-none focus:border-teal-500"
                  />
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  {buscarEnTabla && (
              ...
              <div className="text-right">
                ...
                <p className="text-teal-400 font-bold text-lg">{totalDescargado.toFixed(3)} <span className="text-xs">TM</span></p>
              </div>
            </div>
              ...
              <table className="w-full">
                <thead className="bg-slate-800/50">
                  <tr className="border-b border-white/5">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500">Destino</th>
                    ...
                    viajesPaginados.map((viaje, idx) => {
                      ...
                      return (
                        <tr key={viaje.id} className={`border-b border-white/5 ${bgColor}`}>
                          ...
                          <td className="px-4 py-3 text-teal-400 font-mono font-semibold">{viaje.placa}</td>
        ...
        <div className="bg-slate-800/30 border border-white/5 rounded-xl p-4">
          <div className="flex flex-col gap-2 text-sm text-slate-400">
            <p className="font-bold text-teal-400">PROCESO DE REGISTRO - CLINKER FORTALEZA:</p>
            <p>PASO 1: Registra N° Viaje (editable), Fecha (auto), Hora (manual o "Ahora"), PESO BRUTO, Placa, Tipo, Destino (Bodega/Silo/Bin/Módulo) y Bodega Barco (opcional)</p>
          ...
          <div className="bg-slate-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Select 
                      options={opcionesDestinos} 
                      onChange={(opt) => setViajeEnEdicion({ ...viajeEnEdicion, destino_id: opt?.value || '' })} 
                      value={viajeEnEdicion.destino_id ? opcionesDestinos.find(d => d.value === viajeEnEdicion.destino_id) : null} 
                      placeholder="Seleccionar destino..." 
                      isClearable 
                      styles={selectStyles} 
                    />
            ...
            <div className="flex justify-between gap-3 p-6 border-t border-white/10 sticky bottom-0 bg-slate-800">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={guardarEdicion} className="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
          ...
          <div className="bg-slate-800 rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex justify-end p-6 border-t border-white/10">
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2 rounded-lg font-bold"
              >
                Cerrar
              </button>
            </div>
